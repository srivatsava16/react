...
var DB = d.Databaseconn()

func Createtask(c *gin.Context) {
	var newtask models.Task
	Id, _ := strconv.Atoi(c.PostForm("id"))
	fmt.Println("fsdfsddsffds", Id)
	...
	if err != nil {
		c.JSONP(http.StatusUnprocessableEntity, err.Error())
		return
	}
...
func Monthfilter(c *gin.Context) {
	...
	fmt.Println("valdtteuj", twomonthsback)
	err1 := DB.Model(&models.Task{}).Where("created_at BETWEEN ? AND ?", twomonthsback, pdate).Preload("Employees").Preload("Projects").Find(&monthfilter).Error
	if err1 != nil {
		c.JSON(http.StatusBadRequest, err.Error())
		return
	}
...
func Task5(c *gin.Context) {
	...
	if err := DB.Model(&models.Task{}).Where("created_at  BETWEEN ? AND ?", fromm, too).Preload("Projects", "Employees").Find(&a).Error; err != nil {
		c.JSON(http.StatusBadRequest, err.Error())
