...
func main() {
	...
	fd, err := os.ReadFile("/home/sukshi14/GolandProjects/task1/mJson.json")
	if err != nil {
		log.Println(err)
	}
	...
	for i, val := range manHitData {
		...
		err = writer.WriteField("Fullname", inp.FullName)
		if err != nil {
			log.Println(err)
		}
		err = writer.WriteField("Phone", inp.Phone)
		if err != nil {
			log.Println(err)
		}
		err = writer.WriteField("Email", inp.Email)
		if err != nil {
			log.Println(err)
		}
		err = writer.WriteField("City", inp.City)
		if err != nil {
			log.Println(err)
		}
